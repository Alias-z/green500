...
let classificationsByCik = new Map();
...
const PROCESSABLE_CATEGORIES = new Set(["financial_report", "environment_report", "social_employee", "financial_targets", "climate_targets"]);
...
function countsFor(company, key) {
...
function mergeClassifications(value) {
    return {
        ...value,
...
function updateSectorFilter(companies) {
...
function visibleCompanies() {
    ...
    const sector = element("sector").value;
    const coverageFilter = element("coverage-filter").value;
...
function processingFor(company, category) {
...
function coverageLabel(counts) {
    if ((counts.latest_verified || 0) > 0)
        return { text: "Latest " + counts.latest_verified, className: "ready" };
    if (counts.available > 0)
        return { text: "Files " + counts.available, className: "downloaded" };
    if (counts.pending > 0)
        return { text: "Pending", className: "pending" };
    if (counts.failed > 0)
        return { text: "Issue", className: "issue" };
    return { text: "—", className: "missing" };
}
function categoryCell(company, category) {
...
function scoreCell(company) {
    ...
    if (hasOfficialScore(score)) {
        for (const [name, value] of [["CSA", score.csa_score], ["ESG", score.esg_score]]) {
    ...
    button.onclick = () => {
        ...
        renderScoreDialog();
        element("score-dialog").showModal();
    };
    cell.append(button);
    return cell;
}
function renderScoreDialog() {
    ...
    for (const [label, value] of [["CSA score", score?.csa_score], ["ESG score", score?.esg_score]]) {
    ...
    if (score?.provider_company_name && score.provider_company_name !== company.name)
        entries.push(["Listed company", score.provider_company_name]);
    if (score?.provider_cid)
        entries.push(["S&P company ID", score.provider_cid]);
    if (score?.provider_industry)
        entries.push(["S&P industry", score.provider_industry]);
    ...
    if (score?.assessment_year != null)
        entries.push(["Assessment year", String(score.assessment_year)]);
    if (score?.score_under_review)
        entries.push(["Under review", score.score_under_review]);
...
function renderCompanies() {
    ...
    for (const company of visible) {
        ...
        industry.append(make("strong", company.sector || "—"), make("span", company.sub_industry || company.classification?.sub_industry || "", "company-meta"));
        row.append(identity, industry, scoreCell(company));
        for (const category of categories)
            row.append(categoryCell(company, category));
        body.append(row);
    }
    if (!visible.length) {
        const row = make("tr");
...
function renderCatalog(value) {
    ...
    const processingCount = (category) => ({
    ...
    const setProcessingCount = (id, category) => {
    ...
    const aiActive = setProcessingCount("ai-count", "financial_report")
    ...
    renderHeader();
    renderCompanies();
    if (element("score-dialog").open)
        renderScoreDialog();
}
function datasetAllowed(dataset, category) {
    return (category === "financial_report" && dataset.source_key === "sec_companyfacts")
        || (category === "climate_targets" && dataset.source_key === "sbti");
}
function modalSources(company, category, detail) {
    const values = detail.sources.filter(source => source.categories.includes(category.key)).map(source => ({
    ...
    for (const value of values) {
        ...
        if (!current || (value.documentId || 0) > (current.documentId || 0))
            latestByUrl.set(value.key, value);
    }
    return [...latestByUrl.values()].sort((left, right) => {
        if (Boolean(left.documentId) !== Boolean(right.documentId))
            return left.documentId ? -1 : 1;
        const rank = (source) => source.freshness?.status === "latest_verified" ? 2 : source.freshness?.status === "newer_available" ? 0 : 1;
...
function sourceStatus(source) {
    if (source.status === "provisional")
        return { label: "Provisional", className: "provisional" };
    if (source.documentId) {
        if (["failed", "partial", "unknown", "cancelled"].includes(source.status))
            return { label: "Downloaded · refresh issue", className: "issue" };
        return { label: "Downloaded", className: "downloaded" };
    }
    if (["pending", "running", "not_downloaded"].includes(source.status))
        return { label: "Pending", className: "pending" };
    if (["failed", "partial", "unknown", "cancelled"].includes(source.status))
        return { label: "Issue", className: "issue" };
    return { label: "—", className: "missing" };
}
function freshnessLabel(source) {
    if (source.freshness?.status === "latest_verified")
        return "Latest source verified";
    if (source.freshness?.status === "newer_available")
        return "Newer source available";
    if (source.freshness?.reason === "verification_expired")
        return "Latest-source check expired";
    if (source.freshness?.reason === "content_changed")
        return "Source changed · recheck needed";
    return "Latest source unchecked";
}
function sourceKind(source) {
    if (source.kind === "sec")
        return "SEC financial data";
    if (source.kind === "sbti")
        return "SBTi data";
    const title = source.title.toLowerCase();
    if (title.includes("proxy") || title.includes("def 14a"))
        return "Proxy statement";
    if (title.includes("10-q") || title.includes("quarterly"))
        return "Quarterly filing";
    if (title.includes("10-k") || title.includes("20-f") || title.includes("40-f"))
...
function renderSourceList() {
    ...
    for (const source of dialogState.sources) {
        ...
        button.append(top, make("strong", source.title), make("span", [source.reportingPeriod, formatType(source.contentType)].filter(Boolean).join(" · "), "source-item-meta"));
    ...
    if (!dialogState.sources.length)
        list.append(make("p", "No sources yet.", "empty-message"));
...
function appendEvidence(container, evidence) {
    ...
    for (const entry of entries) {
        if (entry && typeof entry === "object") {
            ...
            list.append(make("li", String(entry)));
    }
    container.append(list);
}
function renderPreviewHeader(source) {
    const header = element("preview-header");
    header.replaceChildren();
    const title = make("div");
    title.append(make("span", sourceKind(source), "overline"), make("h3", source.title), make("p", [source.reportingPeriod, formatType(source.contentType), formatBytes(source.byteCount)].filter(Boolean).join(" · "), "preview-meta"));
    if (source.kind === "report") {
        ...
        if (review?.scope)
            title.append(make("p", "Scope: " + review.scope.replaceAll("_", " "), "preview-meta"));
        ...
        if (assessment) {
            ...
            if (details)
                title.append(make("p", details, "source-assessment"));
        }
    }
    ...
    detailBody.append(make("p", "Status: " + sourceStatus(source).label), make("p", "Checked: " + formatDate(source.checkedAt)), make("p", "Collected via: " + (source.acquisitionMethod || "—")));
    if (source.sha256)
        detailBody.append(make("p", "SHA-256 " + source.sha256, "digest"));
...
async function renderPreview(source) {
    ...
    try {
        ...
        const response = await fetch(useDecodedText ? "/api/catalog/files/" + source.documentId + "/text" : viewUrl, { credentials: "same-origin" });
        if (!response.ok)
        ...
        if (!useDecodedText && type === "application/json") {
            ...
            catch { /* Show malformed provider JSON as inert text. */ }
        }
        if (requestNumber !== previewRequestNumber || previewSignature !== signature)
            return;
        const preview = make("pre", text, "text-preview");
        body.replaceChildren(...(previewNotice ? [previewNotice, preview] : [preview]));
    }
    catch (error) {
        if (requestNumber === previewRequestNumber)
            body.replaceChildren(make("p", "Preview unavailable · " + String(error), "empty-message"));
    }
}
...
async function refreshDialogDetail(selectFirst) {
    if (!dialogState)
        return;
    const state = dialogState;
    const requestNumber = ++detailRequestNumber;
    try {
        const detail = await request("/api/catalog/" + state.company.cik);
        if (requestNumber !== detailRequestNumber || dialogState !== state)
            return;
...
function setReportResultView(show) {
...
function addOriginalResultAction(actions, value) {
...
function renderFinancialResult(panel, value, state) {
    ...
    if (value.processing.status !== "completed")
        identity.append(make("p", "Showing the previous result. The latest attempt needs a retry.", "source-assessment"));
    const actions = make("div", "", "preview-actions");
    ...
    panel.append(header);
    const sourceTitle = data.revenue?.source_document;
    if (sourceTitle)
        panel.append(make("p", sourceTitle, "financial-source-title"));
    ...
    for (const [key, label] of FINANCIAL_METRICS) {
        ...
        if (metric?.notes)
            details.append(make("p", metric.notes));
        if (metric?.source_section)
            details.append(make("p", metric.source_section));
        if (metric?.source_page)
            details.append(make("p", "Page " + metric.source_page));
        const evidence = value.evidence[key];
        if (evidence?.quote)
            details.append(make("blockquote", evidence.quote));
        for (const operand of evidence?.operands || [])
            if (operand.quote)
                details.append(make("blockquote", operand.quote));
        if (metric)
            details.append(make("p", "Extraction confidence: " + Math.round(metric.confidence * 100) + "%"));
        row.append(details);
        metrics.append(row);
    }
...
function appendSourceDetails(container, value) {
    ...
    for (const key of present)
        addProperty(list, key, value[key]);
    details.append(list);
...
function renderRecordCard(label, value, index) {
    ...
    for (const [key, child] of Object.entries(value)) {
        if (SOURCE_DETAIL_KEYS.has(key) || key === "target_name" || key === "metric_name")
            continue;
        if (isRecord(child) || Array.isArray(child))
            nested.push([key, child]);
        else if (child !== null && child !== undefined && child !== "")
            addProperty(properties, key, child);
    }
    if (properties.childElementCount)
        card.append(properties);
    for (const [key, child] of nested)
        appendStructuredValue(card, key, child);
    appendSourceDetails(card, value);
    return card;
}
function appendStructuredValue(container, label, value) {
    ...
    if (isRecord(value)) {
        ...
        for (const [key, child] of Object.entries(value)) {
            if (SOURCE_DETAIL_KEYS.has(key))
                continue;
            if (isRecord(child) || Array.isArray(child))
                appendStructuredValue(group, key, child);
            else if (child !== null && child !== undefined && child !== "")
                addProperty(primitives, key, child);
        }
        if (primitives.childElementCount)
            group.insertBefore(primitives, group.children[1] || null);
    ...
    if (Array.isArray(value)) {
        ...
        if (value.every(child => !isRecord(child) && !Array.isArray(child))) {
            ...
            value.forEach((child, index) => {
                if (isRecord(child))
                    group.append(isMetric(child) ? renderMetric(label + " " + (index + 1), child) : renderRecordCard(label, child, index));
                else
                    appendStructuredValue(group, label + " " + (index + 1), child);
            });
        }
        container.append(group);
        return;
...
function firstNestedString(value, key) {
    if (Array.isArray(value)) {
...
function renderGenericResult(panel, value, state) {
    const data = value.data;
    if (!data)
        return;
    ...
    for (const [key, child] of Object.entries(data)) {
        if (["company", "company_id", "company_name", "reporting_year", "fiscal_year"].includes(key))
            continue;
    ...
    if (groups.childElementCount)
        ...
        panel.append(make("p", "No extracted fields were returned.", "empty-message"));
...
function fixedFieldGroup(field, category) {
    if (category === "financial_targets")
        return field.includes("_long_term_") ? "Long-term targets" : "Annual targets";
    if (category === "climate_targets")
        return "Climate targets";
    if (category === "social_employee")
        return "People and social measures";
    return "Environmental measures";
}
function renderFixedResult(panel, value, state) {
    const data = value.data;
    if (!data?.company || !data.values || !data.metadata)
        return;
    const definitions = value.field_definitions || {};
    const populated = Object.entries(data.values).filter(([, fieldValue]) => fieldValue !== null);
    const header = make("div", "", "report-result-header");
    const identity = make("div");
    identity.append(make("h3", data.company.name + (data.reporting_year ? " · " + data.reporting_year : "")), make("p", [Object.keys(data.values).length + " fixed fields · " + populated.length + " available", value.model, "Processed " + formatDate(value.processed_at)].filter(Boolean).join(" · "), "preview-meta"));
    if (value.processing.status !== "completed")
        identity.append(make("p", "Showing the previous result. The latest attempt needs a retry.", "source-assessment"));
    const actions = make("div", "", "preview-actions");
    const download = make("a", "Download JSON", "button-link");
    ...
    panel.append(header);
    if (data.boundary)
        panel.append(make("p", data.boundary, "fixed-boundary"));
    ...
    for (const [groupName, fields] of groups) {
        ...
        for (const [field, fieldValue, metadata] of fields) {
            ...
            const title = make("div");
            title.append(make("strong", definition?.description || fieldLabel(field)), make("small", [definition?.unit, metadata?.reporting_year].filter(item => item !== null && item !== undefined && item !== "").map(String).join(" · ")));
            ...
            if (metadata) {
                ...
                if (metadata.qualification)
                    detail.append(make("p", metadata.qualification));
                if (metadata.reason)
                    detail.append(make("p", metadata.reason));
                if (metadata.evidence?.quote)
                    detail.append(make("blockquote", metadata.evidence.quote));
                const source = metadata.evidence;
                if (source)
                    detail.append(make("p", "Source block " + source.block_id + (source.raw_value ? " · copied value " + source.raw_value : "")));
                row.append(detail);
            }
            metrics.append(row);
        }
    ...
    if (container.childElementCount)
        ...
        panel.append(make("p", "No source-supported values were extracted.", "empty-message"));
    const raw = make("details", "", "financial-json report-json");
    raw.append(make("summary", "JSON"), make("pre", JSON.stringify(data, null, 2), "text-preview"));
    panel.append(raw);
}
async function refreshReportResult() {
    const state = dialogState;
    if (!state || !reportResultView)
        return;
    ...
    try {
        ...
        if (number !== reportResultRequestNumber || dialogState !== state || !reportResultView)
            return;
        const signature = state.category.key + ":" + JSON.stringify(value);
        if (signature === reportResultSignature)
            return;
        ...
        if (state.category.key === "financial_report")
            renderFinancialResult(panel, value, state);
        else
            renderFixedResult(panel, value, state);
    }
    catch (error) {
        if (error instanceof HttpError && error.status === 401) {
            element("source-dialog").close();
        ...
        else if (number === reportResultRequestNumber)
            panel.replaceChildren(make("p", "AI results unavailable.", "empty-message"));
    }
}
async function openSourceDialog(company, category, showAi = false) {
...
async function refreshCatalog() {
    ...
    try {
        const value = await request("/api/catalog");
        if (requestNumber !== catalogRequestNumber)
            return;
...
async function refreshAll() {
    ...
    if (element("source-dialog").open) {
        if (reportResultView)
            await refreshReportResult();
        else
            await refreshDialogDetail(false);
    }
}
element("login-form").onsubmit = async (event) => {
    event.preventDefault();
    const input = element("viewer-password");
    const submit = element("login-form").querySelector("button[type=submit]");
    if (submit)
...
element("score-close").onclick = () => element("score-dialog").close();
element("score-dialog").addEventListener("close", () => { scoreCompanyCik = null; });
...
async function initialize() {
    try {
        ...
        showLogin(error instanceof HttpError && error.status === 401 ? "" : String(error));
    }
}
void initialize();
